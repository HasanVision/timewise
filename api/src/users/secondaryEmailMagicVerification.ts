...
const magicSecondaryVerifyTokenForEmail: RequestHandler = async (req, res) => {
  ...
  try {
    ...
    const user = await db.user.findUnique({
      where: { secondaryEmail: verificationToken.token },
    });
    ...
    await db.user.update({
      where: { primaryEmail: verificationToken.token },
...
export default magicSecondaryVerifyTokenForEmail;
